...
pub async fn run_gateway(
    ...
    let auth_registry: Arc<dyn AuthRegistry> = {
        ...
        reg.register(Arc::new(LiveA2aPeerProvider::new(Arc::clone(
            &config_state,
        ))));
    ...
    #[cfg(feature = "a2a")]
    let long_running_router = long_running_router.merge(a2a::a2a_task_route().layer(
        crate::auth_middleware::AuthLayer::agent_scoped(auth_registry.clone()),
    ));
...
mod tests {
    ...
    fn admin_paircode_state(
        ...
        AppState {
            ...
            auth_registry: {
                let mut reg = ProviderRegistry::new();
                reg.register(Arc::new(LiveA2aPeerProvider::new(Arc::new(RwLock::new(
                    Config::default(),
                )))));
                Arc::new(LiveAuthRegistry::new(reg))
            },
