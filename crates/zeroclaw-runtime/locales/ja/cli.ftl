...
cli-update-prebuilt-channel-note = ビルド済み更新は軽量なデフォルトチャンネルバンドルを使います。Slack、Discord、その他の非デフォルトチャンネルを使うには、`./install.sh --source --preset full`、`--features channels-full`、または特定の `channel-*` 機能でソースからビルドしてください。
